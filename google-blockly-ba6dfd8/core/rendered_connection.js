...
Blockly.RenderedConnection.prototype.bumpAwayFrom_ = function(staticConnection) {
  if (this.sourceBlock_.workspace.isDragging()) {
    // Don't move custom_blocks around while the user is doing the same.
    return;
  }
  // Move the root block.
  var rootBlock = this.sourceBlock_.getRootBlock();
  if (rootBlock.isInFlyout) {
    // Don't move custom_blocks around in a flyout.
    return;
  }
  var reverse = false;
...
/**
 * Move the custom_blocks on either side of this connection right next to each other.
 * @private
 */
...
Blockly.RenderedConnection.prototype.unhideAll = function() {
  ...
  // All custom_blocks that need unhiding must be unhidden before any rendering takes
  // place, since rendering requires knowing the dimensions of lower custom_blocks.
 ...
 * Render/rerender custom_blocks as needed.
 * @private
 */
 ...
 * block.  Rerender custom_blocks as needed.
