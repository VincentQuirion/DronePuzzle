...
Blockly.Generator.prototype.workspaceToCode = function(workspace) {
  ...
  for (var i = 0, block; block = blocks[i]; i++) {
    ...
    if (Array.isArray(line)) {
      // Value custom_blocks return tuples of code and operator order.
      // Top-level custom_blocks don't care about operator order.
      line = line[0];
    }
    if (line) {
...
Blockly.Generator.prototype.valueToCode = function(block, name, outerOrder) {
  ...
  // Value custom_blocks must return code and order of operations info.
  // Statement custom_blocks must only return code.
  ...
  if (outerOrderClass <= innerOrderClass) {
    if (outerOrderClass == innerOrderClass &&
      ...
      // In all known languages multiple such code custom_blocks are not order
...
/**
 * Common tasks for generating code from custom_blocks.  This is called from
 * blockToCode and is called on every block, not just top level custom_blocks.
 ...
 * @return {string} Code with comments and subsequent custom_blocks added.
 * @private
 */
