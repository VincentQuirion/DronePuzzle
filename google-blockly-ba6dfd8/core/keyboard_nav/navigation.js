...
/**
 * If one of the connections source custom_blocks is a child of the other, disconnect
 * the child.
...
/**
 * If the two custom_blocks are compatible move the moving connection to the target
 ...
 * on the source custom_blocks of the given connections.
 *
 * @param {Blockly.Connection} movingConnection The connection that is being
 *     moved.
 * @param {Blockly.Connection} destConnection The connection to be moved to.
...
Blockly.navigation.connect_ = function(movingConnection, destConnection) {
  ...
  if (movingInferior && destSuperior &&
    ...
    return true;
  // Try swapping the inferior and superior connections on the custom_blocks.
...
/**
 * Disconnect the connection that the cursor is pointing to, and bump custom_blocks.
...
Blockly.navigation.disconnectBlocks_ = function() {
  ...
  if (!curNode.isConnection()) {
    Blockly.navigation.log_('Cannot disconnect custom_blocks when the cursor is not on a connection');
    return;
  }
