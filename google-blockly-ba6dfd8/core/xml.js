...
Blockly.Xml.cloneShadow_ = function(shadow, opt_noId) {
  ...
  while (node) {
    if (opt_noId && node.nodeName == 'shadow') {
      // Strip off IDs from shadow custom_blocks.  There should never be a 'block' as
  ...
  try {
    for (var i = 0; i < childCount; i++) {
      ...
      if (name == 'block' ||
        ...
        // Allow top-level shadow custom_blocks if recordUndo is disabled since
...
Blockly.Xml.appendDomToWorkspace = function(xml, workspace) {
  var bbox;  // Bounding box of the current custom_blocks.
  // First check if we have a workspaceSvg, otherwise the custom_blocks have no shape
  ...
  // Load the new custom_blocks into the workspace and get the IDs of the new custom_blocks.
  ...
  if (bbox && bbox.top != bbox.bottom) {  // check if any previous block
    ...
    // Check position of the new custom_blocks.
    ...
    offsetX = topX - newX;
    // move the new custom_blocks to append them at the bottom
    var width;  // Not used in LTR.
    if (workspace.RTL) {
      width = workspace.getWidth();
    }
...
Blockly.Xml.domToBlockHeadless_ = function(xmlBlock, workspace) {
  ...
  for (var i = 0, xmlChild; xmlChild = xmlBlock.childNodes[i]; i++) {
    ...
    // Find any enclosed custom_blocks or shadows in this tag.
  ...
  if (xmlBlock.nodeName.toLowerCase() == 'shadow') {
    ...
    if (block.getVarModels().length) {
      throw TypeError('Shadow custom_blocks cannot have variable references.');
    }
