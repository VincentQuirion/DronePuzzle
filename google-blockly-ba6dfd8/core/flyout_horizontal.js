...
/**
 * @fileoverview Horizontal flyout tray containing custom_blocks which may be created.
...
/**
 * Lay out the custom_blocks in the flyout.
 * @param {!Array.<!Object>} contents The custom_blocks and buttons to lay out.
 * @param {!Array.<number>} gaps The visible gaps between custom_blocks.
 * @private
 */
  ...
  for (var i = 0, item; (item = contents[i]); i++) {
    if (item.type == 'block') {
      ...
      for (var j = 0, child; (child = allBlocks[j]); j++) {
        // Mark custom_blocks as being inside a flyout.  This is used to detect and
...
Blockly.HorizontalFlyout.prototype.getClientRect = function() {
  ...
  // BIG_NUM is offscreen padding so that custom_blocks dragged beyond the shown flyout
 ...
 * For RTL: Lay out the custom_blocks right-aligned.
 * @private
 */
