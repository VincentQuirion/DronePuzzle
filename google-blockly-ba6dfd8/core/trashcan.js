...
Blockly.Trashcan = function(workspace) {
  ...
  /**
   * A list of XML (stored as strings) representing custom_blocks in the trashcan.
   * @type {!Array.<string>}
   * @private
   */
  this.contents_ = [];
...
/**
 * The openness of the lid when the trashcan contains custom_blocks.
 *    (0.0 = closed, 1.0 = open)
 * @type {number}
 * @private
 */
...
Blockly.Trashcan.prototype.init = function(verticalSpacing) {
  if (this.workspace_.options.maxTrashcanContents > 0) {
    ...
    this.flyout_.isBlockCreatable_ = function() {
      // All custom_blocks, including disabled ones, can be dragged from the
      // trashcan flyout.
      return true;
    };
  }
...
/**
 * Indicate that the trashcan can be clicked (by opening it) if it has custom_blocks.
 * @private
 */
...
/**
 * Handle a BLOCK_DELETE event. Adds deleted custom_blocks oldXml to the content array.
