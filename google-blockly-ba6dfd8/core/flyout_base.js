...
/**
 * @fileoverview Flyout tray containing custom_blocks which may be created.
...
Blockly.Flyout = function(workspaceOptions) {
   ...
   * landing in the custom_blocks' lakes and bays.
  ...
  /**
   * List of custom_blocks that should always be disabled.
...
Blockly.Flyout.prototype.hide = function() {
  ...
  // Do NOT delete the custom_blocks here.  Wait until Flyout.show.
...
Blockly.Flyout.prototype.show = function(xmlList) {
  ...
  // Create the custom_blocks to be shown in this flyout.
  var contents = [];
  var gaps = [];
    ...
    switch (xml.tagName.toUpperCase()) {
      case 'BLOCK':
        ...
        if (!curBlock.isEnabled()) {
          // Record custom_blocks that were initially disabled.
          // Do not enable these custom_blocks as a result of capacity filtering.
  ...
  // When the mouse is over the background, deselect all custom_blocks.
...
/**
 * Delete custom_blocks, mats and buttons from a previous showing of the flyout.
 * @private
 */
Blockly.Flyout.prototype.clearOldBlocks_ = function() {
  // Delete any custom_blocks from a previous showing.
...
Blockly.Flyout.prototype.createRect_ = function(block, x, y, blockHW, index) {
  ...
  // using the block as a button is poor, since custom_blocks have holes in them.
  ...
  // Add the rectangles under the custom_blocks, so that the custom_blocks' tooltips work.
...
/**
 * Filter the custom_blocks on the flyout to disable the ones that are above the
 * capacity limit.  For instance, if the user may only place two more custom_blocks on
 * the workspace, an "a + b" block that has two shadow custom_blocks would be disabled.
 * @private
 */
