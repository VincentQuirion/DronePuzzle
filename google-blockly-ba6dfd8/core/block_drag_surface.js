...
/**
 * @fileoverview A class that manages a surface for dragging custom_blocks.  When a
 * block drag is started, we move the block (and children) to a separate DOM
 * element that we move around using translate3d. At the end of the drag, the
 * custom_blocks are put back in into the SVG they came from. This helps performance by
 * avoiding repainting the entire SVG on every mouse move while dragging custom_blocks.
 * @author picklesrus
 */
...
/**
 * This is where custom_blocks live while they are being dragged if the drag surface
 * is enabled.
 * @type {SVGElement}
 * @private
 */
...
Blockly.BlockDragSurfaceSvg.prototype.setBlocksAndShow = function(blocks) {
  ...
  // appendChild removes the custom_blocks from the previous parent
...
Blockly.BlockDragSurfaceSvg.prototype.translateAndScaleGroup = function(x, y,
    scale) {
  this.scale_ = scale;
  // This is a work-around to prevent a the custom_blocks from rendering
