...
Blockly.WorkspaceSvg = function(options,
  ...
  /**
   * List of currently highlighted custom_blocks.  Block highlighting is often used to
   * visually mark custom_blocks currently being executed.
  ...
  /**
   * The block renderer used for rendering custom_blocks on this workspace.
...
/**
 * Fixed flyout providing custom_blocks which may be dragged into this workspace.
 * @type {Blockly.Flyout}
 * @private
 */
...
/**
 * Category-based toolbox providing custom_blocks which may be dragged into this
 * workspace.
...
/**
 * This workspace's surface for dragging custom_blocks, if it exists.
...
Blockly.WorkspaceSvg.prototype.createDom = function(opt_backgroundClass) {
  ...
  // custom_blocks.  This cannot be changed later, since the UI is very different.
...
Blockly.WorkspaceSvg.prototype.render = function() {
  // Generate list of all custom_blocks.
  var blocks = this.getAllBlocks(false);
  // Render each block.
  for (var i = blocks.length - 1; i >= 0; i--) {
...
Blockly.WorkspaceSvg.prototype.pasteBlock_ = function(xmlBlock) {
  ...
  try {
    ...
    if (!isNaN(blockX) && !isNaN(blockY)) {
      ...
      // distance with neighbouring custom_blocks.
      do {
        var collide = false;
        ...
        if (!collide) {
          // Check for custom_blocks in snap range to any of its connections.
...
Blockly.WorkspaceSvg.prototype.getBlocksBoundingBox = function() {
  ...
  // There are no custom_blocks, return empty rectangle.
...
Blockly.WorkspaceSvg.prototype.showContextMenu_ = function(e) {
  ...
  // Option to clean up custom_blocks.
  if (this.isMovable()) {
    var cleanOption = {};
  ...
  if (this.options.collapse) {
    ...
    /**
     * Option to collapse or expand top custom_blocks.
    ...
    // Option to collapse top custom_blocks.
    ...
    // Option to expand top custom_blocks.
  ...
  // Option to delete all custom_blocks.
  // Count the number of custom_blocks that are deletable.
  var deleteList = [];
...
Blockly.WorkspaceSvg.prototype.zoomCenter = function(type) {
  var metrics = this.getMetrics();
  if (this.flyout_) {
    // If you want custom_blocks in the center of the view (visible portion of the
    ...
    // Note: This only works because of how scrollCenter positions custom_blocks.
...
Blockly.WorkspaceSvg.prototype.zoomToFit = function() {
  if (!this.isMovable()) {
    console.warn('Tried to move a non-movable workspace. This could result' +
        ' in custom_blocks becoming inaccessible.');
    return;
  }
  ...
  if (this.flyout_) {
    ...
    // block size because the custom_blocks we want to resize include the custom_blocks in
...
Blockly.WorkspaceSvg.getContentDimensionsBounded_ = function(ws, svgSize) {
  ...
  // Ensure border is wide enough that custom_blocks can scroll over entire screen.
...
/**
 * Refresh all custom_blocks on the workspace, toolbox and flyout after a theme update.
 * @package
 * @override
 */
