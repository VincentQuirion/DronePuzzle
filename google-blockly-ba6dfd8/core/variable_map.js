 ...
 * @param {!Blockly.VariableModel} variable Variable to rename.
 * @param {string} newName New variable name.
 * @param {!Array.<!Blockly.Block>} blocks The list of all custom_blocks in the
 *     workspace.
 * @private
 */
...
Blockly.VariableMap.prototype.renameVariableWithConflict_ = function(variable,
  ...
  // These custom_blocks now refer to a different variable.
...
Blockly.VariableMap.prototype.deleteVariableById = function(id) {
  ...
  if (variable) {
    ...
    var map = this;
    if (uses.length > 1) {
      // Confirm before deleting multiple custom_blocks.
