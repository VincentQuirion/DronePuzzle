...
/**
 * @fileoverview Toolbox from whence to create custom_blocks.
...
Blockly.Toolbox.prototype.renderTree = function(languageTree) {
  ...
  if (languageTree) {
    ...
    if (this.tree_.blocks.length) {
      throw Error('Toolbox cannot have both custom_blocks and categories ' +
          'in the root level.');
    }
 ...
 * @param {!Node} treeIn DOM tree of custom_blocks.
      ...
      case 'CATEGORY':
        ...
        if (childIn.getAttribute('expanded') == 'true') {
          if (childOut.blocks.length) {
            // This is a category that directly contains custom_blocks.
...
Blockly.Toolbox.prototype.getClientRect = function() {
  ...
  // BIG_NUM is offscreen padding so that custom_blocks dragged beyond the toolbox
...
Blockly.Css.register([
  ...
  '.blocklyToolboxDiv {',
    ...
    'overflow-y: auto;',
    'position: absolute;',
    'z-index: 70;', /* so custom_blocks go under toolbox when dragging */
