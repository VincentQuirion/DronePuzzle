...
/**
 * @fileoverview Components for creating connections between custom_blocks.
...
Blockly.Connection.prototype.connect_ = function(childConnection) {
  ...
  if (parentConnection.isConnected()) {
    ...
    // Displaced shadow custom_blocks dissolve rather than reattaching or bumping.
    if (orphanBlock.isShadow()) {
      ...
      // Statement custom_blocks may be inserted into the middle of a stack.
      // Split the stack.
...
Blockly.Connection.prototype.canConnectToPrevious_ = function(candidate) {
  ...
  // Don't let custom_blocks try to connect to themselves or ones they nest.
...
Blockly.Connection.prototype.isConnectionAllowed = function(candidate) {
  ...
  switch (candidate.type) {
    ...
    case Blockly.NEXT_STATEMENT: {
      // Don't let a block with no next connection bump other custom_blocks out of the
      // stack.  But covering up a shadow block or stack of shadow custom_blocks is
...
/**
 * Walks down a row a custom_blocks, at each stage checking if there are any
 * connections that will accept the orphaned block.  If at any point there
 * are zero or multiple eligible connections, returns null.  Otherwise
 * returns the only input on the last block in the chain.
 * Terminates early for shadow custom_blocks.
 * @param {!Blockly.Block} startBlock The block on which to start the search.
 * @param {!Blockly.Block} orphanBlock The block that is looking for a home.
 * @return {Blockly.Connection} The suitable connection point on the chain
 *    of custom_blocks, or null.
 * @private
 */
