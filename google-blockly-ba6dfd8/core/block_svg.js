...
Blockly.BlockSvg.prototype.select = function() {
  if (this.isShadow() && this.getParent()) {
    // Shadow custom_blocks should not be selected.
...
Blockly.BlockSvg.prototype.moveToDragSurface_ = function() {
  ...
  // The translation for drag surface custom_blocks,
...
Blockly.BlockSvg.prototype.snapToGrid = function() {
  ...
  if (this.workspace.isDragging()) {
    return;  // Don't bump custom_blocks during a drag.
  }
  if (this.getParent()) {
    return;  // Only snap top-level custom_blocks.
  }
  if (this.isInFlyout) {
    return;  // Don't move custom_blocks around in a flyout.
  }
...
Blockly.BlockSvg.prototype.setCollapsed = function(collapsed) {
  ...
  if (collapsed) {
    ...
    // Add any warnings on enclosed custom_blocks to this block.
    ...
    // Clear any warnings inherited from enclosed custom_blocks.
    if (this.warning) {
  ...
  if (!renderList.length) {
    // No child custom_blocks, just render this block.
    renderList[0] = this;
  }
  if (this.rendered) {
...
/**
 * Move the connections for this block and all custom_blocks attached under it.
...
Blockly.BlockSvg.prototype.moveConnections_ = function(dx, dy) {
  if (!this.rendered) {
    // Rendering is required to lay out the custom_blocks.
  ...
  // Recurse through all custom_blocks attached under this one.
...
Blockly.BlockSvg.prototype.updateColour = function() {
  if (!this.isEnabled()) {
    // Disabled custom_blocks don't have colour.
    return;
  }
...
Blockly.BlockSvg.prototype.bumpNeighbours = function() {
  ...
  for (var i = 0, connection; connection = myConnections[i]; i++) {
    ...
    // Spider down from this block bumping all sub-custom_blocks.
    ...
    for (var j = 0, otherConnection; otherConnection = neighbours[j]; j++) {
      ...
      if (!connection.isConnected() || !otherConnection.isConnected()) {
        // Only bump custom_blocks if they are from different tree structures.
...
Blockly.BlockSvg.prototype.render = function(opt_bubble) {
  ...
  if (opt_bubble !== false) {
    // Render all custom_blocks above this one (propagate a reflow).
 ...
 * during rendering.  Also move all of the connected custom_blocks based on the new
...
Blockly.BlockSvg.prototype.getHeightWidth = function() {
  ...
  // Recursively add size of subsequent custom_blocks.
  var nextBlock = this.getNextBlock();
  if (nextBlock) {
