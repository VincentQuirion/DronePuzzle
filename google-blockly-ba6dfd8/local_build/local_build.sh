...
echo Compiling Blockly custom_blocks...
...
# Concatenate all custom_blocks/*.js into the first file, as the compiler will otherwise
# remove them as not needed.
# Also remove 'use strict' to avoid unnecessary warnings
cat ../custom_blocks/*.js| grep -v "^'use strict';" >> temp.js
java -jar $COMPILER \
  --js='temp.js' \
  --generate_exports \
  --warning_level='DEFAULT' \
