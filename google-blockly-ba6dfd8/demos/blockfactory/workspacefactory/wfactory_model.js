 ...
 * element. Also keeps track of all the user-created shadow custom_blocks and
...
WorkspaceFactoryModel = function() {
  ...
  this.toolboxList = [];
  // ListElement for custom_blocks in a single flyout. Null if a toolbox exists.
  this.flyout = new ListElement(ListElement.TYPE_FLYOUT);
  // Array of block IDs for all user created shadow custom_blocks.
  this.shadowBlocks = [];
  // Reference to currently selected ListElement. Stored in this.toolboxList if
  // there are categories, or in this.flyout if custom_blocks are displayed in a single
  // flyout.
 ...
 * Adds a shadow block to the list of shadow custom_blocks.
...
/**
 * Given a set of custom_blocks currently loaded, returns all custom_blocks in the workspace
 * that are user generated shadow custom_blocks.
 * @param {!<Blockly.Block>} custom_blocks Array of custom_blocks currently loaded.
 * @return {!<Blockly.Block>} Array of user-generated shadow custom_blocks currently
 *   loaded.
 */
 ...
 * and the pre-loaded custom_blocks. No duplicates.
...
WorkspaceFactoryModel.prototype.getAllUsedBlockTypes = function() {
  ...
  // Add the block types from any pre-loaded custom_blocks.
 ...
 *    false if none of the custom_blocks are already defined.
 */
