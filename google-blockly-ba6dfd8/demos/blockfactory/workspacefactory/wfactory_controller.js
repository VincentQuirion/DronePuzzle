...
WorkspaceFactoryController = function(toolboxName, toolboxDiv, previewDiv) {
  ...
  // Workspace for user to drag custom_blocks in for a certain category.
  ...
  // Model to keep track of categories and custom_blocks.
  ...
  // True if there are unsaved changes in the preloaded custom_blocks, false otherwise.
...
// udpates the pre-loaded custom_blocks.
...
WorkspaceFactoryController.prototype.addCategory = function() {
  // Transfers the user's custom_blocks to a flyout if it's the first category created.
...
/**
 * Transfers the custom_blocks in the user's flyout to a new category if
...
WorkspaceFactoryController.prototype.transferFlyoutBlocksToCategory =
    function() {
  // Saves the user's custom_blocks from the flyout in a category if there is no
  // toolbox and the user has dragged in custom_blocks.
...
WorkspaceFactoryController.prototype.removeElement = function() {
  ...
  if (!nextId) {
    alert('You currently have no categories or separators. All your custom_blocks' +
...
/**
 * Switches to a new tab for the element given by ID. Stores XML and custom_blocks
...
WorkspaceFactoryController.prototype.clearAndLoadElement = function(id) {
  ...
  if (id != null) {
    ...
    // Order custom_blocks as shown in flyout.
 ...
 * or disabling will cause the user to "drop" their current custom_blocks. Make sure
...
WorkspaceFactoryController.prototype.updatePreview = function() {
  ...
  // Update pre-loaded custom_blocks in the preview workspace.
...
WorkspaceFactoryController.prototype.saveStateFromWorkspace = function() {
  if (this.selectedMode == WorkspaceFactoryController.MODE_TOOLBOX) {
    ...
    // Update flags if preloaded custom_blocks have been changed.
...
WorkspaceFactoryController.prototype.loadCategoryByName = function(name) {
  ...
  // Transfers current flyout custom_blocks to a category if it's the first category
  // created.
  this.transferFlyoutBlocksToCategory();
  ...
  // Convert actual shadow custom_blocks to user-generated shadow custom_blocks.
...
WorkspaceFactoryController.prototype.addSeparator = function() {
  // If adding the first element in the toolbox, transfers the user's custom_blocks
 ...
 * necessary. If the import mode is for pre-loaded custom_blocks in the workspace,
  ...
  reader.onload = function() {
    ...
    try {
      ...
      if (importMode == WorkspaceFactoryController.MODE_TOOLBOX) {
        ...
        // Confirm that the user wants to override their current custom_blocks.
        if (controller.toolboxWorkspace.getAllBlocks(false).length > 0) {
          var msg = 'Are you sure you want to import? You will lose your ' +
            'current workspace custom_blocks.';
...
WorkspaceFactoryController.prototype.importToolboxFromTree_ = function(tree) {
  ...
  if (tree.getElementsByTagName('category').length == 0) {
    ...
    // Load all the custom_blocks into a single category evenly spaced.
    ...
    for (var i = 0, item; item = tree.children[i]; i++) {
      ...
      if (item.tagName == 'category') {
        ...
        // Load all custom_blocks in that category to the workspace to be evenly
        ...
        // Evenly space the custom_blocks.
        this.toolboxWorkspace.cleanUp();
 ...
 * custom_blocks in the model and view and all pre-loaded custom_blocks. Tied to the
 * "Clear" button.
 */
 ...
 * custom_blocks are not made into real shadow custom_blocks, but recorded in the model
 * and visually marked as shadow custom_blocks, allowing the user to move and edit
 * them (which would be impossible with actual shadow custom_blocks). Updates the
 * preview when done.
 */
WorkspaceFactoryController.prototype.addShadow = function() {
  ...
  // Set selected block and all children as shadow custom_blocks.
...
/**
 * Sets a block and all of its children to be user-generated shadow custom_blocks,
...
WorkspaceFactoryController.prototype.addShadowForBlockAndChildren_ =
  ...
  if (FactoryUtils.hasVariableField(block)) {
    block.setWarningText('Cannot make variable custom_blocks shadow custom_blocks.');
  }

  // Convert all children to shadow custom_blocks recursively.
 ...
 * shadow custom_blocks and loading the workspace again. Updates the preview again.
 */
...
/**
 * Call when importing XML containing real shadow custom_blocks. This function turns
 * all real shadow custom_blocks loaded in the workspace into user-generated shadow
 * custom_blocks, meaning they are marked as shadow custom_blocks by the model and appear as
 * shadow custom_blocks in the view but are still editable and movable.
 */
...
/**
 * Clears the toolbox workspace and loads XML to it, marking shadow custom_blocks
 * as necessary.
 * @private
...
/**
 * Imports custom_blocks from a file, generating a category in the toolbox workspace
 * to allow the user to use imported custom_blocks in the toolbox and in pre-loaded
 * custom_blocks.
 * @param {!File} file File object for the custom_blocks to import.
...
WorkspaceFactoryController.prototype.importBlocks = function(file, format) {
  ...
  reader.onload = function() {
    try {
      // Define custom_blocks using block types from file.
      ...
      // Reload current category to possibly reflect any newly defined custom_blocks.
      ...
      msg = 'Cannot read custom_blocks from file.';
      alert(msg);
      BlocklyDevTools.Analytics.onError(msg);
...
WorkspaceFactoryController.prototype.setBlockLibCategory =
  ...
  // Reload XML on page to account for custom_blocks now defined or undefined in block
  // library.
...
/**
 * Sets a warning on custom_blocks loaded to the workspace that are not defined.
 * @private
 */
