...
WorkspaceFactoryInit.addWorkspaceFactoryEventListeners_ = function(controller) {
  ...
  controller.toolboxWorkspace.addChangeListener(function(e) {
    ...
    // custom_blocks when dragging them into workspace. Could cause problems if ever
    // load custom_blocks into workspace directly without calling updatePreview.
    ...
    // surrounding parent, meaning it is nested in another block (custom_blocks that
    // are not nested in parents cannot be shadow custom_blocks).
      ...
      if (selected != null && selected.getSurroundParent() != null &&
        ...
        if (selected != null && isInvalidBlockPlacement(selected)) {
          ...
          if (!controller.isUserGenShadowBlock(selected.id)) {
            ...
            selected.setWarningText('Only shadow custom_blocks can be nested inside\n'
                + 'other shadow custom_blocks.');
            ...
            selected.setWarningText('Shadow custom_blocks must be nested inside other'
                + ' custom_blocks.')
          }
    ...
    // Convert actual shadow custom_blocks added from the toolbox to user-generated
    // shadow custom_blocks.
    if (e.type == Blockly.Events.BLOCK_CREATE) {
      controller.convertShadowBlocks();

      // Let the user create a Variables or Functions category if they use
      // custom_blocks from either category.
      ...
      // If any of the newly created custom_blocks are variable or procedure custom_blocks,
