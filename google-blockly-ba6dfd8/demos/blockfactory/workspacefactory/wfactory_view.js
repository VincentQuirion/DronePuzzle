 ...
 * to/from separators where the user shouldn't be able to drag custom_blocks into
 * the workspace.
...
/**
 * Given a set of custom_blocks currently loaded user-generated shadow custom_blocks, visually
 * marks them without making them actual shadow custom_blocks (allowing them to still
 * be editable and movable).
 * @param {!Array.<!Blockly.Block>} blocks Array of user-generated shadow custom_blocks
 * currently loaded.
 */
...
WorkspaceFactoryView.prototype.markShadowBlock = function(block) {
  // Add Blockly CSS for user-generated shadow custom_blocks.
  Blockly.utils.dom.addClass(block.svgGroup_, 'shadowBlock');
  // If not a valid shadow block, add a warning message.
  if (!block.getSurroundParent()) {
      block.setWarningText('Shadow custom_blocks must be nested inside' +
          ' other custom_blocks to be displayed.');
  }
...
WorkspaceFactoryView.prototype.unmarkShadowBlock = function(block) {
  // Remove Blockly CSS for user-generated shadow custom_blocks.
...
WorkspaceFactoryView.prototype.updateHelpText = function(mode) {
  if (mode == WorkspaceFactoryController.MODE_TOOLBOX) {
    var helpText = 'Drag custom_blocks into the workspace to configure the toolbox ' +
        'in your custom workspace.';
  } else {
    var helpText = 'Drag custom_blocks into the workspace to pre-load them in your ' +
        'custom workspace.'
  }
 ...
 * or a single flyout of custom_blocks. Updates checkboxes and text fields.
 */
WorkspaceFactoryView.prototype.setBaseOptions = function() {
  ...
  // Check infinite custom_blocks and hide suboption.
 ...
 *    custom_blocks are displayed in a single flyout.
 */
