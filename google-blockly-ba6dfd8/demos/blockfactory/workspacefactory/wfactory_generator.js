...
WorkspaceFactoryGenerator = function(model) {
  // Model to share information about categories and shadow custom_blocks.
  this.model = model;
 ...
 * custom_blocks are added by user to be part of the toolbox.
...
WorkspaceFactoryGenerator.prototype.generateToolboxXml = function() {
  ...
  if (!this.model.hasElements()) {
    ...
    // that all the custom_blocks that are not top custom_blocks are also captured as block
    ...
    for (var i = 0; i < toolboxList.length; i++) {
      ...
      if (element.type == ListElement.TYPE_SEPARATOR) {
        ...
        // custom_blocks as real shadow custom_blocks.
  ...
  * generated shadow custom_blocks to actual shadow custom_blocks.
  * @return {!Element} XML element representing toolbox or flyout corresponding
  * to toolbox workspace.
  */
WorkspaceFactoryGenerator.prototype.generateWorkspaceXml = function() {
  // Load workspace XML to hidden workspace with user-generated shadow custom_blocks
  // as actual shadow custom_blocks.
...
WorkspaceFactoryGenerator.prototype.generateInjectString = function() {
  ...
  finalStr += '\n\n/* Load Workspace Blocks from XML to workspace. ' +
      'Remove all code below if no custom_blocks to load */\n\n' +
      '/* TODO: Change workspace custom_blocks XML ID if necessary. Can export' +
      ' workspace custom_blocks XML from Workspace Factory. */\n' +
      'var workspaceBlocks = document.getElementById("workspaceBlocks"); \n\n' +
      '/* Load custom_blocks to workspace. */\n' +
 ...
 * shadow custom_blocks to be actual shadow custom_blocks.
...
/**
 * Encodes custom_blocks in the hidden workspace in a XML DOM element. Very
 * similar to workspaceToDom, but doesn't capture IDs. Uses the top-level
 * custom_blocks loaded in hiddenWorkspace.
 * @private
...
/**
 * Sets the user-generated shadow custom_blocks loaded into hiddenWorkspace to be
 * actual shadow custom_blocks. This is done so that blockToDom records them as
 * shadow custom_blocks instead of regular custom_blocks.
 * @private
 */
 ...
 * type.
 * @param {!Array.<!Element>} blockTypes Array of custom_blocks that have been defined.
