 ...
 *     or 'Dart'.
 * @return {string} Generator code for multiple custom_blocks.
 */
FactoryUtils.getGeneratorStub = function(block, generatorLanguage) {
  // Build factory custom_blocks from block
...
/**
 * Generates a category containing custom_blocks of the specified block types.
...
/**
 * Define custom_blocks from imported block definitions.
...
FactoryUtils.defineAndGetBlockTypes = function(blockDefsString, format) {
  ...
  // Define custom_blocks and get block types.
  if (format == 'JSON') {
...
/**
 * Returns whether or not two custom_blocks are the same based on their XML. Expects
 ...
 *    is a factory_base block.
 * @return {boolean} Whether or not two custom_blocks are the same based on their XML.
 */
 ...
 * 'structure' of the custom_blocks (i.e. block order, field values, etc).
 ...
 * other known markers for procedure custom_blocks beyond name).
 ...
 *    Controller storing custom custom_blocks.
