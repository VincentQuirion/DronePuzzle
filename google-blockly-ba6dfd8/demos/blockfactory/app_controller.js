...
AppController.prototype.onTab = function() {
  ...
  if (this.selectedTab == AppController.EXPORTER) {
    ...
    // Need accurate state in order to know which custom_blocks are used in workspace
    // factory.
    this.workspaceFactoryController.saveStateFromWorkspace();

    // Update exporter's list of the types of custom_blocks used in workspace factory.
    ...
    // Update the exporter's preview to reflect any changes made to the custom_blocks.
...
AppController.prototype.assignExporterClickHandlers = function() {
  ...
  // Export custom_blocks when the user submits the export settings.
...
AppController.prototype.addBlockFactoryEventListeners = function() {
  ...
  // Disable custom_blocks not attached to the factory_base block.
...
AppController.prototype.initializeBlocklyStorage = function() {
  ...
  BlocklyStorage.LINK_ALERT =
      'Share your custom_blocks with this link:\n\n%1';
