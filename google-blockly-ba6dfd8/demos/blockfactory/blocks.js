...
Blockly.Blocks['factory_base'] = {
  ...
  init: function() {
    ...
    this.setTooltip('Build a custom block by plugging\n' +
        'fields, inputs and other custom_blocks here.');
    this.setHelpUrl(
  ...
  spawnOutputShadow_: function(option) {
    ...
    // to attach shaddow custom_blocks to.
    switch (option) {
      case 'LEFT':
...
Blockly.Blocks['type_group'] = {
  ...
  saveConnections: function(containerBlock) {
    // Store a pointer to any connected child custom_blocks.
    var typeBlock = containerBlock.getInputTargetBlock('STACK');
    var i = 0;
    while (typeBlock) {
...
function fieldNameCheck(referenceBlock) {
  ...
  var msg = (count > 1) ?
      'There are ' + count + ' field custom_blocks\n with this name.' : null;
...
function inputNameCheck(referenceBlock) {
  ...
  var msg = (count > 1) ?
      'There are ' + count + ' input custom_blocks\n with this name.' : null;
  referenceBlock.setWarningText(msg);
}
