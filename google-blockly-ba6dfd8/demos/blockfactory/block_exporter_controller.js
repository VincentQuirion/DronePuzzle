 ...
 * users to export block definitions and generator stubs of their saved custom_blocks
...
function BlockExporterController(blockLibStorage) {
  // BlockLibrary.Storage object containing user's saved custom_blocks.
...
/**
 * Get selected custom_blocks from block selector, pulls info from the Export
 * Settings form in Block Exporter, and downloads code accordingly.
 */
BlockExporterController.prototype.export = function() {
  // Get selected custom_blocks' information.
  ...
  if (wantBlockDef) {
    // User wants to export selected custom_blocks' definitions.
    if (!blockDef_filename) {
      ...
      // Get block definition code in the selected format for the custom_blocks.
  ...
  if (wantGenStub) {
    // User wants to export selected custom_blocks' generator stubs.
    if (!generatorStub_filename) {
      ...
      // Get generator stub code in the selected language for the custom_blocks.
...
/**
 * Update the Exporter's block selector with block options generated from custom_blocks
...
BlockExporterController.prototype.updateSelector = function() {
  ...
  // Select all previously selected custom_blocks.
 ...
 * Deselects all custom_blocks in the selector and updates text accordingly.
 */
 ...
 * Selects all custom_blocks stored in block library for export.
 */
...
/**
 * Returns the category XML containing all custom_blocks in the block library.
...
BlockExporterController.prototype.addBlockOptionSelectHandlers = function() {
  ...
  var updateSelectedBlockTypes_ = function(blockOption) {
    ...
    // Show currently selected custom_blocks in helper text.
 ...
 * Selects all custom_blocks stored in block library and used in workspace factory.
 */
BlockExporterController.prototype.selectUsedBlocks = function() {
  // Deselect all custom_blocks.
  this.view.deselectAllBlocks();
  ...
  if (unstoredCustomBlockTypes.length > 0) {
    // Warn user to import block definitions and generator code for custom_blocks
    // not in their Block Library nor Blockly's standard library.
    var blockTypesText = unstoredCustomBlockTypes.join(', ');
    var customWarning = 'Custom custom_blocks used in workspace factory but not ' +
        'stored in block library:\n ' + blockTypesText +
        '\n\nDon\'t forget to include block definitions and generator code ' +
        'for these custom_blocks.';
    alert(customWarning);
  }
};
 ...
 * preview to reflect selected custom_blocks.
 */
BlockExporterController.prototype.updatePreview = function() {
  // Generate preview code for selected custom_blocks.
...
/**
 * Get block definition code in the selected format for selected custom_blocks.
...
/**
 * Get generator stubs in the selected language for selected custom_blocks.
